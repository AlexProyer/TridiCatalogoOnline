...
Esta guía es para ti, sin necesidad de saber programar. Explica cómo entrar
al panel, agregar un producto, cambiar un precio y subir fotos.

## Entrar al panel

1. Abre en el navegador: **https://tridicatalogo.alexbuitrago156.workers.dev/admin/**
2. Haz clic en **"Login with GitHub"**.
3. Se abre una ventana de GitHub pidiendo autorizar el acceso — inicia sesión
   con tu cuenta de GitHub si te lo pide, y acepta.
4. Vas a ver el panel con una sección llamada **"Productos"** en el menú de
   la izquierda.

Si alguna vez ves un mensaje de error al iniciar sesión, cierra la ventana e
intenta de nuevo. Si persiste, revisa con quien te ayudó a configurar el
...
1. Cuando editas algo y le das **"Save" / "Guardar"**, el cambio queda
   guardado como **borrador**, todavía no se ve en la página real.
2. Para que el cambio se vea en el sitio en vivo, tienes que ir a la sección
...
Esto es a propósito: te da la oportunidad de revisar antes de que un error
de tipeo (un precio mal puesto, por ejemplo) se vea en el sitio real.

## Agregar un producto nuevo

1. Entra a **"Productos"** en el menú de la izquierda.
...
3. Completa los campos:
   ...
   - **Foto principal**: la foto que se ve en la grilla del catálogo (ver
     abajo "Subir fotos").
   - **Colores disponibles**: agrega uno o más colores, cada uno con:
     - Nombre del color (ej: "Blanco").
     - El color en sí (eliges de una paleta o escribes el código).
...
1. Entra a **"Productos"**.
2. Haz clic en el producto que quieras editar (se identifican por nombre y precio en la lista).
3. Cambia el campo que necesites (por ejemplo, "Precio (COP)").
...
1. Haz clic en el campo de la foto — se abre una ventana para elegir una
   imagen.
2. Puedes **subir una foto nueva desde tu computadora** (botón para elegir
...
**Recomendación**: antes de subir una foto, ponle un nombre descriptivo en
tu computadora (por ejemplo `taza-personalizada-azul.jpg` en vez de
`IMG_2034.jpg`). Todas las fotos que subas desde el panel quedan juntas en
una misma carpeta del sitio, así que un nombre claro ayuda a encontrarlas
después si hace falta.

## Quitar un producto

1. Entra a **"Productos"**, abre el producto que quieras eliminar.
2. Busca la opción de eliminar ese elemento de la lista (un ícono de
   tacho de basura o "Remove", normalmente arriba a la derecha de ese
   producto dentro del formulario).
3. Guarda y publica los cambios como siempre.

## ¿Y si algo sale mal?

- Si guardaste un cambio por error y todavía no lo publicaste, puedes
  entrar al borrador pendiente y editarlo de nuevo antes de publicar, o
  descartarlo.
- Si publicaste algo por error, simplemente vuelve a editar ese mismo campo
  con el valor correcto, guarda y publica de nuevo — es como cualquier
  corrección normal.
